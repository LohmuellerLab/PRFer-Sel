...
foreach $i (0..3*1000) {
	$gamma = 10 ** ($i/1000);
	open OUT1, ">setting_positive-human-ns";
	print OUT1 "fixed = 1\n";
	...
	system("./exec_fname setting_positive-human-ns out_positive_s-human-ns");
	
	open IN, "out_positive_s-human-ns" or die "cannot open prfreq outfile";
	my $line1 = <IN>;
	my $line2 = <IN>;
